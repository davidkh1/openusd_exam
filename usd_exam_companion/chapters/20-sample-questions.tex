...
\begin{FieldNote}
Treat this chapter like the real thing at least once: no references, a
timer running, and fast elimination of obviously-wrong options. Certified
test-takers report time pressure as a bigger enemy than difficulty ---
discarding distractors quickly is a trainable skill, and these questions
are the training set.
\end{FieldNote}

\noindent Answers, with worked explanations, begin on
page~\pageref{sec:sample-key}.
...
\Question{3}
\noindent\textbf{Domain:} Data Modeling\par
\vspace{0.4em}
...
\vspace{0.4em}
\noindent\textbf{Question:} When creating a procedural mesh in USD, which attributes must be kept synchronized to ensure the mesh remains valid? \textit{(Select two options.)}
\begin{Choices}
...
\Question{6}
\noindent\textbf{Domain:} Customizing USD\par
\vspace{0.4em}
...
\begin{Choices}
  ...
  \item D.\ Layers improve scene performance by optimizing data organization and access.
\end{Choices}
...
\Question{9}
\noindent\textbf{Domain:} Visualization\par
\vspace{0.4em}
...
\noindent\textbf{Domain:} Visualization\par
\vspace{0.4em}
\noindent\textbf{Question:} You want to create a material that can be easily customized with different colors and textures. Which USD shading approach would be most appropriate?
\begin{Choices}
  ...
  \item C.\ Create separate materials for each color/texture combination.
...
\vspace{0.4em}
\noindent\textbf{Question:} You reference a USD file with \texttt{metersPerUnit = 1.0} (meters) into a stage with \texttt{metersPerUnit = 0.01} (centimeters). What happens?
\begin{Choices}
...
\begin{Choices}
  ...
  \item B.\ The local opinion for the \texttt{color} selection in
        \texttt{main.usda} is stronger than the opinion authored by
        \texttt{loftedColor}.
  \item C.\ \texttt{loftedColor} does not set \texttt{primvars:displayColor}, so changing it cannot affect color.
