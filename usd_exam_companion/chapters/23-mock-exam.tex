...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ \texttt{.usd} may contain either text or crate; \texttt{usdcat} reveals which.
  \item C.\ \texttt{.usd} is always text; \texttt{.usdc} is the only binary form.
  \item D.\ The encoding can only be determined by a hex editor.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ Default traversal does not descend into instances; use a range with \texttt{Usd.TraverseInstanceProxies()}.
  \item C.\ Meshes inside instances lose their typed schema.
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ A component under a parent with no group kind is not part of the model hierarchy.
  \item C.\ \texttt{subcomponent} prims are models like any other.
  \item D.\ Kinds are validated at authoring time; invalid hierarchies cannot be saved.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ Temporarily removing the layer from composition for this stage --- a debugging bisection tool.
  \item C.\ Preventing the layer from being saved.
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ Reading, traversal, and inspection --- authoring raises an error.
  \item C.\ Adding new child prims beneath it.
  \item D.\ Changing its composition arcs.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  \item A.\ A \texttt{UsdUVTexture}'s \texttt{rgb} output feeding \texttt{diffuseColor}.
  \item B.\ A \texttt{float3} primvar reader targeting
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ Flatten the stage (and package with \texttt{usdzip} if textures travel along).
  \item C.\ Mute all internal layers before saving.
  \item D.\ Re-author all arcs as inherits.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ Many small Sdf-level edits are batched so change processing runs once.
  \item C.\ The stage is opened with \texttt{LoadNone}.
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ A \texttt{plugInfo.json} registering it with a base kind of
        \texttt{component}, discoverable on the plugin path at every
        consuming site.
  \item C.\ A custom typed schema deriving from \texttt{UsdTyped}.
  \item D.\ Recompiling USD with the new kind.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  \item A.\ Setting the value on the proxy path with \texttt{Set()}.
  \item B.\ Editing the prototype's source asset (or an \texttt{over} on it in a stronger layer), so every instance updates.
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ Strongest opinion first.
  \item C.\ Weakest opinion first.
  \item D.\ In file-modification order.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ Consumers depend on a stable interface while contents change underneath; heavy data stays deferrable.
  \item C.\ The payload arc renders faster than a reference.
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ Three failed checks from the stage-metadata rules.
  \item C.\ Nothing --- those are optional conveniences.
  \item D.\ A single combined warning about style.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ USD's C++ ABI changes between releases; mismatched builds fail to load (often silently).
  \item C.\ plugInfo.json embeds a checksum of the USD libraries.
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ Composition structure: references, variants, payloads, layer offsets.
  \item C.\ Face counts.
  \item D.\ UV sets.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ A new layer of \texttt{over}s composed above the original via sublayer or reference.
  \item C.\ Flattening the import into the asset file.
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ They encode the same scene description; crate is memory-mapped and lazily read, text is diffable.
  \item C.\ usda loads faster for large scenes.
  \item D.\ usdc files cannot be converted back to usda.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ One shared material whose \texttt{UsdPrimvarReader} consumes a per-prim primvar.
  \item C.\ De-instancing all props and baking vertex colors.
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ The mesh topology changes (triangulation, deleted faces) --- subsets index faces by position.
  \item C.\ The mesh is moved in world space.
  \item D.\ Another subset family is added.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ Same namespace (opinions about the same prims) vs grafting a subtree under a new path.
  \item C.\ Whether the data is animated.
  \item D.\ Number of files involved.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ \texttt{invisible} on an ancestor hides the whole subtree.
  \item C.\ It only affects proxy-purpose prims.
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ USD n-gons must be triangulated for glTF, and up-axis/units must be converted (glTF is Y-up, meters).
  \item C.\ glTF cannot store UVs.
  \item D.\ USD and glTF share the same material model exactly.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ Wrap the authoring loop in \texttt{Sdf.ChangeBlock} (writing at the Sdf level, reading only after).
  \item C.\ Run with \texttt{TF\_DEBUG=USD\_CHANGES} permanently enabled.
  \item D.\ Re-open the stage between edits.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ Hides the listed instances without touching positions/protoIndices --- cheap and reversible.
  \item C.\ Hides whole prototypes.
  \item D.\ Is only honored by usdview.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  \item A.\ It does not; they are interchangeable.
  \item B.\ Roles drive transformation (points translate, vectors do not) and color management.
...
\begin{itemize}[leftmargin=*]
  ...
  \item B.\ \texttt{intensity}, \texttt{color}, \texttt{exposure}.
  \item C.\ \texttt{shadow:color} only.
...
\begin{itemize}[leftmargin=*]
  \item A.\ Spend up to five minutes per question to be safe.
  \item B.\ Eliminate clearly-wrong options fast, mark uncertain questions, and return --- about 90 seconds per question on average.
